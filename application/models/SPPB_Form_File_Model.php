<?php
class SPPB_Form_File_Model extends CI_Model{
	...
	function file_list_by_ID_SPPB_FORM($ID_SPPB_FORM){
		$hasil=$this->db->query("SELECT * FROM sppb_form_file WHERE ID_SPPB_FORM = '$ID_SPPB_FORM' ORDER BY TANGGAL_UPLOAD ASC");
		return $hasil;
	}

	function file_list_by_ID_SPPB_FORM_result($ID_SPPB_FORM){
		$hasil=$this->db->query("SELECT * FROM sppb_form_file WHERE ID_SPPB_FORM = '$ID_SPPB_FORM' ORDER BY TANGGAL_UPLOAD ASC");
		return $hasil->result();
	}

	function file_list_by_HASH_MD5_SPPB($HASH_MD5_SPPB){
		$hasil=$this->db->query("SELECT * FROM sppb_form_file WHERE HASH_MD5_SPPB = '$HASH_MD5_SPPB' ORDER BY TANGGAL_UPLOAD ASC");
		return $hasil;
	}

	function file_list_by_HASH_MD5_SPPB_result($HASH_MD5_SPPB){
		$hasil=$this->db->query("SELECT * FROM sppb_form_file WHERE HASH_MD5_SPPB = '$HASH_MD5_SPPB' ORDER BY TANGGAL_UPLOAD ASC");
		return $hasil->result();
	}

	function file_list_by_DOK_FILE($DOK_FILE){
		$hasil=$this->db->query("SELECT * FROM sppb_form_file WHERE DOK_file = '$DOK_FILE' ORDER BY TANGGAL_UPLOAD ASC");
		return $hasil;
	}

	function hapus_data_by_DOK_FILE($DOK_FILE){
		$hasil=$this->db->query("DELETE FROM sppb_form_file WHERE DOK_file='$DOK_FILE'");
		return $hasil;
	}
