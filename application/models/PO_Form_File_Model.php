<?php
class PO_Form_File_Model extends CI_Model{
	...
	function file_list_by_HASH_MD5_PO($HASH_MD5_PO){
		$hasil=$this->db->query("SELECT * FROM po_form_file WHERE HASH_MD5_PO = '$HASH_MD5_PO' ORDER BY TANGGAL_UPLOAD ASC");
		return $hasil;
	}

	function file_list_by_HASH_MD5_PO_result($HASH_MD5_PO){
		$hasil=$this->db->query("SELECT * FROM po_form_file WHERE HASH_MD5_PO = '$HASH_MD5_PO' ORDER BY TANGGAL_UPLOAD ASC");
		return $hasil->result();
	}
