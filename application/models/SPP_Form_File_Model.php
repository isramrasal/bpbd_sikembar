<?php
class SPP_Form_File_Model extends CI_Model{
	...
	function file_list_by_ID_SPP_FORM($ID_SPP_FORM){
		$hasil=$this->db->query("SELECT * FROM spp_form_file WHERE ID_SPP_FORM = '$ID_SPP_FORM' ORDER BY TANGGAL_UPLOAD ASC");
		return $hasil;
	}

	function file_list_by_ID_SPP_FORM_result($ID_SPP_FORM){
		$hasil=$this->db->query("SELECT * FROM spp_form_file WHERE ID_SPP_FORM = '$ID_SPP_FORM' ORDER BY TANGGAL_UPLOAD ASC");
		return $hasil->result();
	}

	function file_list_by_HASH_MD5_SPP($HASH_MD5_SPP){
		$hasil=$this->db->query("SELECT * FROM spp_form_file WHERE HASH_MD5_SPP = '$HASH_MD5_SPP' ORDER BY TANGGAL_UPLOAD ASC");
		return $hasil;
	}

	function file_list_by_HASH_MD5_SPP_result($HASH_MD5_SPP){
		$hasil=$this->db->query("SELECT * FROM spp_form_file WHERE HASH_MD5_SPP = '$HASH_MD5_SPP' ORDER BY TANGGAL_UPLOAD ASC");
		return $hasil->result();
	}

	function file_list_by_DOK_FILE($DOK_FILE){
		$hasil=$this->db->query("SELECT * FROM spp_form_file WHERE DOK_file = '$DOK_FILE' ORDER BY TANGGAL_UPLOAD ASC");
		return $hasil;
	}
