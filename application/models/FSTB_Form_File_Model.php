<?php
class FSTB_Form_File_Model extends CI_Model{
	...
	function file_list_by_ID_FSTB_FORM($ID_FSTB_FORM){
		$hasil=$this->db->query("SELECT * FROM FSTB_form_file WHERE ID_FSTB_FORM = '$ID_FSTB_FORM' ORDER BY TANGGAL_UPLOAD ASC");
		return $hasil;
	}

	function file_list_by_ID_FSTB_FORM_result($ID_FSTB_FORM){
		$hasil=$this->db->query("SELECT * FROM FSTB_form_file WHERE ID_FSTB_FORM = '$ID_FSTB_FORM' ORDER BY TANGGAL_UPLOAD ASC");
		return $hasil->result();
	}

	function file_list_by_HASH_MD5_FSTB($HASH_MD5_FSTB){
		$hasil=$this->db->query("SELECT * FROM FSTB_form_file WHERE HASH_MD5_FSTB = '$HASH_MD5_FSTB' ORDER BY TANGGAL_UPLOAD ASC");
		return $hasil;
	}

	function file_list_by_HASH_MD5_FSTB_result($HASH_MD5_FSTB){
		$hasil=$this->db->query("SELECT * FROM FSTB_form_file WHERE HASH_MD5_FSTB = '$HASH_MD5_FSTB' ORDER BY TANGGAL_UPLOAD ASC");
		return $hasil->result();
	}

	function file_list_by_DOK_FILE($DOK_FILE){
		$hasil=$this->db->query("SELECT * FROM FSTB_form_file WHERE DOK_file = '$DOK_FILE' ORDER BY TANGGAL_UPLOAD ASC");
		return $hasil;
	}

	function hapus_data_by_DOK_FILE($DOK_FILE){
		$hasil=$this->db->query("DELETE FROM FSTB_form_file WHERE DOK_file='$DOK_FILE'");
		return $hasil;
	}
