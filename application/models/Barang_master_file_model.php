<?php
class Barang_master_file_model extends CI_Model{
	...
	function file_list_by_HASH_MD5_BARANG_MASTER($HASH_MD5_BARANG_MASTER){
		$hasil=$this->db->query("SELECT * FROM barang_master_file WHERE HASH_MD5_BARANG_MASTER = '$HASH_MD5_BARANG_MASTER' ORDER BY TANGGAL_UPLOAD ASC");
		return $hasil;
	}

	function file_list_by_HASH_MD5_BARANG_MASTER_result($HASH_MD5_BARANG_MASTER){
		$hasil=$this->db->query("SELECT * FROM barang_master_file WHERE HASH_MD5_BARANG_MASTER = '$HASH_MD5_BARANG_MASTER' ORDER BY TANGGAL_UPLOAD ASC");
		return $hasil->result();
	}
	...
	function hapus_data_by_DOK_FILE($DOK_FILE){
		$hasil=$this->db->query("DELETE FROM barang_master_file WHERE DOK_file='$DOK_FILE'");
		return $hasil;
	}
