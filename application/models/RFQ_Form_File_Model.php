<?php
class RFQ_Form_File_Model extends CI_Model{
	...
	function file_list_by_HASH_MD5_RFQ($HASH_MD5_RFQ){
		$hasil=$this->db->query("SELECT * FROM rfq_form_file WHERE HASH_MD5_RFQ = '$HASH_MD5_RFQ' ORDER BY TANGGAL_UPLOAD ASC");
		return $hasil;
	}

	function file_list_by_HASH_MD5_RFQ_result($HASH_MD5_RFQ){
		$hasil=$this->db->query("SELECT * FROM rfq_form_file WHERE HASH_MD5_RFQ = '$HASH_MD5_RFQ' ORDER BY TANGGAL_UPLOAD ASC");
		return $hasil->result();
	}
