<?php
class Vendor_Invoice_Form_File_Model extends CI_Model{
	...
	function file_list_by_ID_PO_FORM($ID_PO_FORM){
		$hasil=$this->db->query("SELECT * FROM po_form_file WHERE ID_PO_FORM = '$ID_PO_FORM' ORDER BY TANGGAL_UPLOAD ASC");
		return $hasil;
	}

	function file_list_by_ID_PO_FORM_result($ID_PO_FORM){
		$hasil=$this->db->query("SELECT * FROM po_form_file WHERE ID_PO_FORM = '$ID_PO_FORM' ORDER BY TANGGAL_UPLOAD ASC");
		return $hasil->result();
	}

	function file_list_by_HASH_MD5_VENDOR_INVOICE($HASH_MD5_VENDOR_INVOICE){
		$hasil=$this->db->query("SELECT * FROM vendor_invoice_form_file WHERE HASH_MD5_VENDOR_INVOICE = '$HASH_MD5_VENDOR_INVOICE' ORDER BY TANGGAL_UPLOAD ASC");
		return $hasil;
	}

	function file_list_by_HASH_MD5_VENDOR_INVOICE_result($HASH_MD5_VENDOR_INVOICE){
		$hasil=$this->db->query("SELECT * FROM vendor_invoice_form_file WHERE HASH_MD5_VENDOR_INVOICE = '$HASH_MD5_VENDOR_INVOICE' ORDER BY TANGGAL_UPLOAD ASC");
		return $hasil->result();
	}

	function file_list_by_DOK_FILE($DOK_FILE){
		$hasil=$this->db->query("SELECT * FROM po_form_file WHERE DOK_file = '$DOK_FILE' ORDER BY TANGGAL_UPLOAD ASC");
		return $hasil;
	}

	function hapus_data_by_DOK_FILE($DOK_FILE){
		$hasil=$this->db->query("DELETE FROM po_form_file WHERE DOK_file='$DOK_FILE'");
		return $hasil;
	}
