<?php
class Proyek_file_model extends CI_Model{
	...
	function file_list_by_id_proyek($ID_PROYEK){
		$hasil=$this->db->query("SELECT * FROM proyek_file WHERE ID_PROYEK = '$ID_PROYEK' ORDER BY TANGGAL_UPLOAD ASC");
		return $hasil;
	}

	function file_list_by_HASH_MD5_PROYEK($HASH_MD5_PROYEK){
		$hasil=$this->db->query("SELECT * FROM proyek_file WHERE HASH_MD5_PROYEK = '$HASH_MD5_PROYEK' ORDER BY TANGGAL_UPLOAD ASC");
		return $hasil;
	}

	function file_list_by_HASH_MD5_PROYEK_result($HASH_MD5_PROYEK){
		$hasil=$this->db->query("SELECT * FROM proyek_file WHERE HASH_MD5_PROYEK = '$HASH_MD5_PROYEK' ORDER BY TANGGAL_UPLOAD ASC");
		return $hasil->result();
	}

	function file_list_by_DOK_FILE($DOK_FILE){
		$hasil=$this->db->query("SELECT * FROM proyek_file WHERE DOK_file = '$DOK_FILE' ORDER BY TANGGAL_UPLOAD ASC");
		return $hasil;
	}
	...
	function hapus_data_by_DOK_FILE($DOK_FILE){
		$hasil=$this->db->query("DELETE FROM proyek_file WHERE DOK_file='$DOK_FILE'");
		return $hasil;
	}
