        ...
        <div class="modal-content animated bounceInRight">
            ...
            <div class="form-horizontal">
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label class="col-xs-3 control-label">Tanggal Pengajuan SPP</label>
                        <div class="col-xs-9">
                            <input id="TANGGAL_DOKUMEN_SPP" type="date" class="form-control">
...
<script>
    $(document).ready(function() {
        ...
        document.querySelector("#TANGGAL_DOKUMEN_SPP").valueAsDate = new Date();
                        ...
                        $.ajax({
                            ...
                            success: function(data) {
                                ...
                                html += '<tr>' +
                                    ...
                                    '<td>' + '<a href="#" class="btn btn-link btn-xs text-decoration-none"> ' + data_1[i].TANGGAL_PEMBUATAN_SPPB_HARI + ' </a>' + '</td>' +
                                ...
                                if (data_2 == 'TIDAK ADA DATA') {
                                    ...
                                    for (j = 0; j < data_2.length; j++) {
                                        var html_progress = '';
                                        if (data_2[j].PROGRESS_SPP == "Dalam Proses Manajer Kantor Pusat") {
                                            html_progress = '<a href="#" class="btn btn-warning btn-xs"> ' + data_2[j].PROGRESS_SPP + ' </a>';
                                        } else if (data_2[j].PROGRESS_SPP == "Dalam Proses Pembuatan Manajer Procurement KP") {
                                            html_progress = '<a href="#" class="btn btn-warning btn-xs"> ' + data_2[j].PROGRESS_SPP + ' </a>';
                                        } else if (data_2[j].PROGRESS_SPP == "SPP Disetujui") {
                                            html_progress = '<a href="#" class="btn btn-primary btn-xs"> ' + data_2[j].PROGRESS_SPP + ' </a>';
                                        } else {
                        ...
                        $.ajax({
                            ...
                            success: function(data) {
                                $.each(data, function() {
                                    ...
                                    $('[name="NO_URUT_SPP"]').val(`${NO_URUT}/SPP/WME/${INISIAL}/${date.getFullYear()}`);
                                    $('[name="FILE_NAME_TEMP"]').val(`${NO_URUT}_SPP_WME_${INISIAL}_${date.getFullYear()}`);
        ...
        $('#btn_simpan').click(function() {
            var form_data = {
                ...
                TANGGAL_DOKUMEN_SPP: $('#TANGGAL_DOKUMEN_SPP').val()
            };
            $.ajax({
