            ...
            <div class="ibox float-e-margins" id="ibox1">
                ...
                <div class="ibox-content">
                    ...
                    <a href="#" class="btn btn-primary btn-xs" name="btn_buat" id="btn_buat"><span
                            class="fa fa-plus"></span> Buat Penyaluran Bantuan</a>
        ...
        <div class="col-lg-12">
            <div class="ibox float-e-margins" id="ibox1_pengajuan">
                ...
                <div class="ibox-content">
                    ...
                    <!-- List jenis bencana -->
                    <select class="chosen-select" name="ID_JENIS_BENCANA_LIST_PENGAJUAN" class="form-control"
                        id="ID_JENIS_BENCANA_LIST_PENGAJUAN">
                        <option value=''>- Pilih Bencana -</option>
                        <option value='Semua'>Semua Bencana</option>
                        <option value='Gempa Bumi'>Gempa Bumi</option>
                        <option value='Angin Puting Beliung'>Angin Puting Beliung</option>
...
$(document).ready(function() {
                ...
                success: function(data) {
                    ...
                    if (data.length > 0) {
                        for (i = 0; i < data.length; i++) {
                            ...
                            html += '<tr>' +
                                '<td>' +
                                '<a href="<?php echo base_url() ?>Penyaluran_form/view/' +
                                data[i].CODE_MD5 +
                                ...
                                data[i].CODE_MD5 + '"><i class="fa fa-pencil"></i>' + data[
                                    i].STATUS_SPPB + '</a>' + '</td>' +
                                '</tr>';
                        }
                    } else {
                    ...
                    html_head =
                        '<tr><th>No. Penyaluran</th><th>Jenis Bencana</th><th>Nama Pegawai</th><th>Nama Penerima</th><th>NIK Penerima</th><th>Instansi Penerima</th><th>Desa Penerima</th><th>Kecamatan Penerima</th><th>Tanggal Kejadian Bencana</th><th>Aksi</th></tr>';
                    $('#show_data_head').html(html_head);
                    $('#show_data').html(html);
    ...
    $("#ID_JENIS_BENCANA_LIST_PENGAJUAN").change(function() {
        var form_data = {
            ID_JENIS_BENCANA_LIST: $('#ID_JENIS_BENCANA_LIST_PENGAJUAN').val()
        };
        var ID_JENIS_BENCANA_LIST = $('#ID_JENIS_BENCANA_LIST_PENGAJUAN').val();
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data && Array.isArray(data) && data.length > 0) {
                    for (var i = 0; i < data.length; i++) {
                        html += '<tr>' +
                            ...
                            '<td>' + data[i].Kecamatan_Bencana + '</td>' +
                            '<td>' + data[i].Desa_Kelurahan_Bencana + '</td>' +
                            '<td>' + (data[i].TANGGAL_KEJADIAN_BENCANA === '0000-00-00' ?
                                '-' : data[i].TANGGAL_KEJADIAN_BENCANA) + '</td>' +
                            '<td>' + (data[i].Tanggal_Pembuatan === '0000-00-00' ? '-' :
                                data[i].Tanggal_Pembuatan) + '</td>' +
                            '<td>' +
                            '<a href="javascript:;" class="btn btn-success btn-xs item_status block" data="' +
                            data[i].CODE_MD5 + '"><i class="fa fa-pencil"></i> ' + data[i]
                            .STATUS_PENGAJUAN + '</a>' +
                            '</td>' +
                            '</tr>';
                    }
                ...
                $('#mydata_pengajuan').DataTable({
                    ...
                    columns: [{
                            ...
                            render: function(data) {
                                return `<a href="<?php echo base_url(); ?>pengajuan_form/view/${data}" 
                        ...
                        {
                            data: 'Kecamatan_Bencana',
                            title: 'Kecamatan'
                        },
                        {
                            data: 'Desa_Kelurahan_Bencana',
                            title: 'Desa'
                        },
                        {
                            data: 'TANGGAL_KEJADIAN_BENCANA',
                        ...
                        },
                        {
                            ...
                            render: function(data, type, row) {
                                return `<a href="javascript:;" class="btn btn-success btn-xs item_status block" 
                            data="${row.CODE_MD5}">
                            <i class="fa fa-pencil"></i> ${data}
                        </a>`;
                            }
                        }
                    ],
                    ...
                    aaSorting: [], // Nonaktifkan pengurutan otomatis
                ...
                // Matikan loading animation
                $('#ibox1_pengajuan').children('.ibox-content').toggleClass('sk-loading');
            },
            error: function(xhr, status, error) {
                console.error("Error mengambil data:", error); // Debugging error
                console.log("Status:", status);
                console.log("XHR:", xhr);

                // Tampilkan pesan error di tabel
                $('#show_data_pengajuan').html(
