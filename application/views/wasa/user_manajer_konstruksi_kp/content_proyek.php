						...
						<table class="table table-striped table-bordered table-hover" id="mydata">
							<thead>
								<tr>
									...
									<th>Nama Supervisor Logistik</th>
									<th>Nama Supervisor Procurement</th>
									<th>Pilihan</th>
								</tr>
							</thead>
							<tbody id="show_data">
...
<script>
	$(document).ready(function() {
		...
		tampil_data_proyek(); //pemanggilan fungsi tampil data.
		...
		function tampil_data_proyek() {
			$.ajax({
				type: 'ajax',
				url: '<?php echo base_url() ?>index.php/proyek/data_proyek',
				async: false,
				dataType: 'json',
				success: function(data) {
					...
					for (i = 0; i < data.length; i++) {
						...
						html += '<tr>' +
							...
							'<td>' + data[i].PEGAWAI_LOG + '</td>' +
							'<td>' + data[i].PEGAWAI_PROC + '</td>' +
							'<td>' +
							'<a href="<?php echo base_url() ?>proyek/profil_proyek/'+data[i].HASH_MD5_PROYEK +'" class="btn btn-warning btn-xs btn-outline block"><i class="fa fa-eye"></i> Lihat Data </a>' + ' ' +
