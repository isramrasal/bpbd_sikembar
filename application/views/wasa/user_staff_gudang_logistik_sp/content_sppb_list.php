                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-xs-9">
                            ...
                            //     echo '<input name="FPB[] id="chkbx_fpb_' . $i . '" value="' . $LIST_FPB->NO_URUT_FPB . '" type="checkbox" " />' . $LIST_FPB->NO_URUT_FPB . '</br>';
                            //     $i = $i + 1;
                            //var_dump($LIST_FPB);die;
                            // } 
                            ?>
                            <div id="mlCheckbox">
                                <?php $i = 1;
                                foreach ($LIST_FPB as $lb) : ?>
                                    <input type="checkbox" id="list_fpb<?= $i++; ?>" value="<?= $lb->NO_URUT_FPB ?>">
                                    <?= $lb->NO_URUT_FPB ?><br>
                                <?php endforeach; ?>
                            </div>
                        </div>
                    ...
                    <div id="alert-msg_1">Silakan langsung tekan Buat SPPB untuk membuat SPPB dari lembar kosong </div>
...
<script>
        ...
        $.ajax({
            ...
            success: function(data) {
                $.each(data, function() {
                    ...
                    $('[name="FILE_NAME_TEMP"]').val(`${NO_URUT}_SPPB_WME_${INISIAL}_${jenis}_2021`);
                });
    ...
    $(document).ready(function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    for (i = 0; i < data.length; i++) {
                        ...
                        let PROGRESS_SPPB = data[i].PROGRESS_SPPB;
                        let STATUS_SPPB = data[i].STATUS_SPPB;

                        // if (PROGRESS_SPPB != "Dalam Proses Staff Logistic") {
                        //     tombol_ubah = '<a href="#" class="btn btn-warning btn-xs block" disabled><i class="fa fa-pencil"></i> Ubah </a>';
                        // } else {
                        //     tombol_ubah = '<a href="<?php echo base_url() ?>sppb_form/index/' + data[i].HASH_MD5_SPPB + '" class="btn btn-warning btn-xs block"><i class="fa fa-pencil"></i> Ubah </a>';
                        // }

                        if (data[i].PROGRESS_SPPB == "SPPB Disetujui") {
        ...
        $('#btn_simpan').click(function() {
            ...
            <?php $i = 1;
            foreach ($LIST_FPB as $lb) : ?>
                if ($('#list_fpb<?= $i; ?>').is(':checked')) {
                    LIST_FPB.push($('#list_fpb<?= $i; ?>').val())
                }
                <?php $i++; ?>
            <?php endforeach; ?>
            form_data.LIST_FPB = LIST_FPB;
