			...
			$.ajax({
				...
				success: function(data) {
					...
					for (i = 0; i < data.length; i++) {
						$.ajax({
							url: "<?php echo site_url('RASD/get_list_rasd_by_id_proyek') ?>",
							type: "POST",
							dataType: "JSON",
							async: false,
							data: {
							...
							success: function(data) {
								...
								if (data_2 == 'TIDAK ADA DATA') {
									...
									for (j = 0; j < data_2.length; j++) {
										var html_progress = '';
										html_progress = '<a href="<?php echo base_url() ?>rasd_form/index/' + data_2[j].HASH_MD5_RASD + '" class="btn btn-warning btn-xs block"><i class="fa fa-pencil"></i> Ubah RASD '+ data_2[j].NAMA_SUB_PEKERJAAN+' </a>' + ' ';
										html += html_progress;
									}
