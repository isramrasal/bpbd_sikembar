                    ...
                    <div class="form-group">
                        ...
                        <div class="col-xs-9" id="data_1">
                            <div class="input-group">
                                <span class="input-group-addon"><i class="fa fa-calendar"></i></span><input id="TANGGAL_DOKUMEN_SPPB" type="date" class="form-control unstyled" />
                            </div>
                            (mm/dd/yyyy)
                        </div>
                    </div>
...
<script>
    $('#ModalAdd').on('shown.bs.modal', function(e) {
        ...
        // var pisah = ID_DEPARTEMEN_PEGAWAI.split(' ');
        ...
        $.ajax({
            ...
            success: function(data) {
                $.each(data, function() {
                    ...
                    $('[name="NO_URUT_FPB"]').val(`${NO_URUT}/FPB/WME/${INISIAL}/${SUB_DEPARTEMEN}/2022`);
                    $('[name="FILE_NAME_TEMP"]').val(`${NO_URUT}_FPB_WME_${INISIAL}_${SUB_DEPARTEMEN}_2022`);
                });
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    for (i = 0; i < data.length; i++) {
                        ...
                        var PROGRESS_FPB = data[i].PROGRESS_FPB;
                        var STATUS_FPB = data[i].STATUS_FPB;
                        var due_date = '';

                        if (data[i].PROGRESS_FPB == "Dalam Proses Pembuatan SM") {
                        ...
                        } else if (data[i].PROGRESS_FPB == "Dalam Proses Approval Chief-SM") {
                            html_progress = '<a href="#" class="btn btn-primary btn-xs block"> ' + data[i].PROGRESS_FPB + ' </a>';
                            tr_kode = '<tr>';
                            tombol_fpb = '<a href="<?php echo base_url() ?>FPB_form/view/' + data[i].HASH_MD5_FPB + '" class="btn btn-warning btn-xs block"><i class="fa fa-eye"></i> Lihat FPB </a>';

                            if (data[i].DUE_DATE_SM != null) {
                                
                                var tandaminus = data[i].SELISIH_WAKTU_DUE_DATE_SM[0];
                                if (tandaminus == '-') {
                                    due_date = '</br> <h1 style="font-size:10px"> Batas akhir proses: </br> <b>' + data[i].DUE_DATE_CHIEF + '</b></h1>';
                                } else {
                                    due_date = '';
                                }
                            } else {
                        ...
                        html += tr_kode +
                            ...
                            '<td>' + data[i].STATUS_FPB + due_date + '</td>' +
                            '<td>' + tombol_fpb  + ' ' +
                            '</td>' +
                            '</tr>';
                    }
