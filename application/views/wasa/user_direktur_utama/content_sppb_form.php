            ...
            <div class="form-horizontal">
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <div class="checkbox i-checks"><label> <input type="checkbox" id="saya_setuju"><i></i> Saya telah selesai melakukan pengisian form SPPB ini dan menyetujui untuk diproses lebih lanjut </label></div>
                    </div>
...
<script>
    ...
    $(document).ready(function() {
        if (document.getElementById('dropzoneForm')) {
            ...
            file_upload.on("removedfile", function(a) {
                ...
                $.ajax({
                    ...
                    url: "<?php echo base_url('index.php/SPPB_form/remove_file') ?>",
                    cache: false,
                    dataType: 'json',
                    success: function() {
        ...
        item_edit_kirim_email_sppb.onclick = function() {
            var HASH_MD5_SPPB = $(this).attr('data');
            $.ajax({
                type: "GET",
                url: "<?php echo base_url('SPPB_form/get_id_sppb_by_HASH_MD5_SPPB') ?>",
                dataType: "JSON",
                data: {
                ...
                success: function(data) {
                    ...
                    if (data.ID_VENDOR == "0" || data.TOP == "" || data.LOKASI_PENYERAHAN == "") {
                        ...
                        $('#alert-msg-7').html('<div>Anda belum menentukan Vendor atau Term of Payment atau Lokasi Penyerahan. Anda dialihkan ke halaman pengisian SPPB</div>');
                        ...
                        alert("Anda belum menentukan Vendor atau Term of Payment atau Lokasi Penyerahan. Anda dialihkan ke halaman pengisian SPPB"); //called second
                        var alamat = "<?php echo base_url('SPPB_form/index/'); ?>" + data.HASH_MD5_SPPB;
                        window.location = alamat;
                    } else {
                        $('#ModalEditKirimSPPB').modal('show');
                        $('[name="HASH_MD5_SPPB7"]').val(data.HASH_MD5_SPPB);
                    }
        ...
        $('#btn_update_kirim_sppb').on('click', function() {
            ...
            let HASH_MD5_SPPB = $('#ID_SPPB7').val();
