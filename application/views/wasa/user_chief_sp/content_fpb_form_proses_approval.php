...
<div class="wrapper wrapper-content animated fadeInRight">
    ...
    <div class="ibox float-e-margins">
        <div class="ibox-title">
            <h5>Approval Formulir Pengisian Item Barang/Jasa FPB</h5>
                        ...
                        <table class="table table-striped table-bordered table-hover" id="mydata">
                            <thead>
                                <tr>
                                    ...
                                    <th>Qty Tersedia di Gudang <?php echo $NAMA_PROYEK; ?> </th>
                                    <th>Qty Diajukan SPPB </th>
                                        ...
                                        foreach ($rasd_barang_list as $data) : ?>
                                            <tr>
                                                ...
                                                <?php if ($data->ID_BARANG_MASTER == null) { ?>
                                                    <td><span class="label label-warning block"><i class="fa fa-warning"></i> Data Baru</span></td>
                                                <?php } else { ?>
...
<div class="modal inmodal fade" id="ModalEditJustifikasiBarang" tabindex="-1" role="dialog" aria-labelledby="largeModal" aria-hidden="true">
    <div class="modal-dialog" style="width: 40vw;">
        <div class="modal-content animated bounceInRight">
            ...
            echo form_open("FPB_form/update_data_justifikasi_barang", $attributes); ?>
            <div class="form-horizontal">
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-xs-9">
                            <textarea class="form-control h-200" name="JUSTIFIKASI_CHIEF5" id="JUSTIFIKASI_CHIEF5" placeholder="Contoh: " required></textarea>
                        </div>
                    </div>
            ...
            <div class="form-horizontal">
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-xs-9">
                            <textarea class="form-control h-200" name="CTT_CHIEF6" id="CTT_CHIEF6" required></textarea>
                        </div>
                    </div>
        ...
        <div class="modal-content animated bounceInRight">
            ...
            <div class="form-horizontal">
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <div class="checkbox i-checks"><label> <input type="checkbox" id="saya_setuju"><i></i> Saya telah selesai melakukan proses form FPB ini dan menyetujui untuk diproses lebih lanjut </label></div>
                    </div>
    ...
    <div class="modal-dialog" role="document" style="width: 40vw;">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="myModalLabel">Tolak Item Barang/Jasa FPB</h4>
            </div>
...
<script>
    $(document).ready(function() {
        ...
        tampil_data_fpb_form_approval(); //pemanggilan fungsi tampil data.
        ...
        //fungsi tampil data
        function tampil_data_fpb_form_approval() {
            $.ajax({
                type: 'GET',
                url: '<?php echo base_url() ?>FPB_form/data_fpb_form_appproval',
                async: false,
                dataType: 'json',
                data: {
                    id: ID_FPB
                    ...
                    for (i = 0; i < data.length; i++) {
                        ...
                        if (data[i].CORET == '1') {
                            ...
                            html += '<tr>' +
                                ...
                                '<td class="danger"> <strike>' + data[i].NAMA_SATUAN_BARANG + '</td> </strike>' +
                                '<td class="danger"> <strike>' + data[i].JUMLAH_MINTA + '</td> </strike>' +
                                '<td class="danger"> <strike>' + data[i].JUMLAH_QTY_GUDANG + '</td> </strike>' +
                                '<td class="danger"> <strike>' + data[i].JUMLAH_QTY_SPPB + '</td> </strike>' +
                                '<td class="danger"> <strike>' + data[i].TANGGAL_MULAI_PAKAI + '</td> </strike>' +
                                '<td class="danger"> <strike>' + data[i].TANGGAL_SELESAI_PAKAI + '</td> </strike>' +
                                '<td>' +
                                '<a href="javascript:;" class="btn btn-info btn-xs item_batal_coret block" data="' + data[i].ID_FPB_FORM + '"><i class="fa fa-check"></i> Batal Tolak</a>' + ' ' + data[i].CATATAN_CORET +
                                '</td>' +
                                '<td>' +
                                ' ' + data[i].REMARKS_CHIEF + ' ' + data[i].REMARKS_STAFF_GUDANG_LOGISTIK + ' ' + data[i].REMARKS_SM + ' ' + data[i].REMARKS_STAFF_UMUM_LOGISTIK +
                                '</td>' +
                            ...
                            html += '<tr>' +
                                ...
                                '<td>' + data[i].JUMLAH_MINTA + '</td>' +
                                '<td>' + data[i].JUMLAH_QTY_GUDANG + '</td>' +
                                '<td>' + data[i].JUMLAH_QTY_SPPB + '</td>' +
                                '<td>' + data[i].TANGGAL_MULAI_PAKAI + '</td>' +
                                '<td>' + data[i].TANGGAL_SELESAI_PAKAI + '</td>' +
                                '<td>' +
                                '<a href="javascript:;" class="btn btn-danger btn-xs item_coret block" data="' + data[i].ID_FPB_FORM + '"><i class="fa fa-ban"></i> Tolak</a>' + ' ' + data[i].CATATAN_BATAL_CORET +
                                '<td>' +
                                ' ' + data[i].REMARKS_CHIEF + ' ' + data[i].REMARKS_STAFF_GUDANG_LOGISTIK + ' ' + data[i].REMARKS_SM + ' ' + data[i].REMARKS_STAFF_UMUM_LOGISTIK +
                                '</td>' +
        ...
        $('#show_data_approval').on('click', '.item_edit', function() {
        ...
        $('#show_data_approval').on('click', '.item_edit_justifikasi', function() {
            var id = $(this).attr('data');
            $.ajax({
                type: "GET",
                url: "<?php echo base_url('FPB_form/get_data') ?>",
            ...
            $.ajax({
                ...
                success: function(data) {
                    $.each(data, function() {
                        ...
                        $('[name="CTT_CHIEF6"]').val(data.CTT_CHIEF);
        ...
        //GET CORET
        $('#show_data_approval').on('click', '.item_coret', function() {
            var id = $(this).attr('data');
            $.ajax({
                type: "GET",
                url: "<?php echo base_url('FPB_form/get_data') ?>",
        ...
        //GET BATAL
        $('#show_data_approval').on('click', '.item_batal_coret', function() {
            var id = $(this).attr('data');
            $.ajax({
                type: "GET",
                url: "<?php echo base_url('FPB_form/get_data') ?>",
        ...
        $('#btn_update_justifikasi_barang').on('click', function() {
            ...
            $.ajax({
                url: "<?php echo site_url('FPB_form/update_data_justifikasi_barang') ?>",
                type: "POST",
                dataType: "JSON",
                data: {
                    ID_FPB_FORM: ID_FPB_FORM,
        ...
        //UPDATE CATATAN FPB APPROVAL
        $('#btn_update_catatan_fpb').on('click', function() {

            let ID_FPB = $('#ID_FPB6').val();
            let CTT_CHIEF = $('#CTT_CHIEF6').val();
            $.ajax({
                url: "<?php echo site_url('FPB_form/update_data_catatan_fpb_approval') ?>",
                type: "POST",
                dataType: "JSON",
                data: {
                    ID_FPB: ID_FPB,
        ...
        $('#btn_update_kirim_fpb').on('click', function() {
            ...
            $.ajax({
                url: "<?php echo site_url('FPB_form/update_data_kirim_fpb_approval') ?>",
                type: "POST",
                dataType: "JSON",
                data: {
                    ID_FPB: ID_FPB,
        ...
        $('#btn_coret').on('click', function() {
            ...
            $.ajax({
                type: "POST",
                url: "<?php echo base_url('FPB_form/update_data_coret') ?>",
                dataType: "JSON",
                data: form_data,
                success: function(data) {
                    if (data == true) {
        ...
        $('#btn_terima').on('click', function() {
            ...
            $.ajax({
                type: "POST",
                url: "<?php echo base_url('FPB_form/update_data_batal_coret') ?>",
                dataType: "JSON",
                data: form_data,
                success: function(data) {
                    if (data == true) {
