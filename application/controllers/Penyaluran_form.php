...
class Penyaluran_form extends CI_Controller
{
	...
	public function index() //BEDA KP DAN SP
	{
		...
		if ($this->ion_auth->logged_in() && $this->ion_auth->in_group(2)) { //User bpbd
			...
			if ($PROGRESS_PENGAJUAN == "Diproses oleh Staff BPBD") {
				...
				$this->load->view('wasa/user_pegawai_bpbd/content_penyaluran_form_proses');
	...
	function data_penyaluran_barang_form() //102023
	{
		if ($this->ion_auth->logged_in()) {
			$ID_FORM_INVENTARIS_PENYALURAN_BARANG_BENCANA = $this->input->post('ID_FORM_INVENTARIS_PENYALURAN_BARANG_BENCANA');
			$data = $this->Penyaluran_form_model->data_penyaluran_barang_form($ID_FORM_INVENTARIS_PENYALURAN_BARANG_BENCANA);
			echo json_encode($data);
		} else {
			$this->logout();
			$this->session->set_flashdata('message', 'Anda tidak memiliki otorisasi untuk mengakses sistem, silahkan hubungi admin');
	...
	function simpan_data_barang_bantuan()
	{
		if ($this->ion_auth->logged_in()) {
			...
			if ($this->form_validation->run() == FALSE) { //validation fails
				...
				$data = $this->Penyaluran_form_model->simpan_data_penyaluran_barang(
