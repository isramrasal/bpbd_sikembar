...
class Data_Korban_form extends CI_Controller
{
	...
	public function __construct()
	{
		...
		$this->lang->load('auth');
		$this->data['title'] = 'SiKembar | Form Data Korban';

		$this->load->model('FPB_form_model');
		$this->load->model('SPPB_form_model');
		$this->load->model('Data_Korban_form_model');
	...
	public function index() //BEDA KP DAN SP
	{
		...
		if ($this->ion_auth->logged_in() && $this->ion_auth->in_group(3)) { //User korban
			$hasil_2 = $this->Data_Korban_model->get_data_korban_by_CODE_MD5($CODE_MD5);
			$PROGRESS_DATA_KORBAN = $hasil_2['PROGRESS_DATA_KORBAN'];
			// $sess_data['HASH_MD5_SPPB'] = $HASH_MD5_SPPB;
			// $this->session->set_userdata($sess_data);

			if ($PROGRESS_DATA_KORBAN == "Diproses oleh Staff BPBD") {
				$hasil = $this->Data_Korban_model->get_data_korban_by_CODE_MD5($CODE_MD5);
				$ID_KORBAN = $hasil['ID_KORBAN'];
				$this->data['CODE_MD5'] = $CODE_MD5;
				$this->data['ID_KORBAN'] = $ID_KORBAN;
				
				// $this->data['ID_SPPB'] = $ID_SPPB;
				$this->data['Korban'] = $this->Data_Korban_model->korban_list_by_id_korban($ID_KORBAN);
				...
				$this->load->view('wasa/user_korban_bencana/content_data_korban_form_proses');
				// $this->load->view('wasa/user_korban_bencana/footer');
			} else {
				redirect('Data_Korban', 'refresh');
			}
	...
	function data_sppb_form() //102023
	{
		if ($this->ion_auth->logged_in()) {
			$ID_SPPB = $this->input->get('ID_SPPB');
			$ID_RAB_FORM = $this->input->get('ID_RAB_FORM');
			$data = $this->SPPB_form_model->sppb_form_list_by_id_sppb($ID_SPPB, $ID_RAB_FORM);
			echo json_encode($data);
	...
	function simpan_data_sppb_pembelian_1_item()
	{
		if ($this->ion_auth->logged_in()) {
			...
			if ($ID_RAB_FORM == '999999999') {
				...
				$this->form_validation->set_rules('JUMLAH_QTY_SPP', 'Jumlah Barang', 'trim|numeric|required|greater_than[0]|less_than[99999999999]');
				...
				if ($this->form_validation->run() == FALSE) { //validation fails
					...
					if ($this->RAB_form_model->cek_nama_kategori_rab($ID_RAB, $NAMA_KATEGORI) == 'Data belum ada') {
						...
						$data = $this->SPPB_form_model->simpan_data_spbb_pembelian_1_item_by_rab_and_by_rasd(
							$ID_SPPB,
							$SATUAN_BARANG,
							$ID_RASD_FORM,
							$ID_RAB_FORM,
